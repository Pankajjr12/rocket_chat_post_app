...
const Icons = ({ post }) => {
  ...
  // Function to handle sharing post with a user
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Update the saved state and post list
      if (!saved) {
        const updatedPosts = posts.map((p) => {
          if (p._id === post._id) {
            return { ...p, savedBy: [...p.savedBy, user._id] };
          }
          return p;
        });
        setPosts(updatedPosts);
        ...
        const updatedPosts = posts.map((p) => {
          if (p._id === post._id) {
            return { ...p, savedBy: p.savedBy.filter((id) => id !== user._id) };
          }
          return p;
        });
        setPosts(updatedPosts);
  ...
  const handleUnSave = async () => {
    if (!user) return showToast("Error", "You must be logged in to unsave a post", "error");

    try {
      const res = await fetch(`/api/posts/unsave/${post._id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      setPosts(updatedPosts);
      showToast("Success", "Post unsaved successfully 👍", "success");
      setSaved(false);
    } catch (error) {
      showToast("Error", error.message, "error");
    }
        ...
        <Flex>
          {saved ? (
            <MdBookmark
              ...
              color=""
              size="20"
              onClick={handleUnSave} // Handle unsave
            />
          ) : (
            <MdBookmarkBorder
              aria-label="Save"
